...
The neuroscience bridge here is evidence accumulation, and it is worth drawing precisely. A drift-diffusion model integrates noisy sensory evidence over time to form a decision variable — the accumulated evidence for one option versus another. Early samples contribute, but late samples dominate because they shift the decision variable from a region where the choice is uncertain to one where it is resolved. The user tower is doing the same thing: integrating noisy behavioral signals (a click is weak evidence of preference, a purchase is strong evidence) over time to form a taste representation. Recent signals matter more not because old signals are discarded, but because the accumulated state has already absorbed them and the decision variable is closer to convergence. A new interaction shifts the state by a smaller amount when it confirms existing taste (drift toward the boundary) and a larger amount when it contradicts it (evidence against the accumulated direction).

The difference from a DDM is that the user tower's "decision variable" is not scalar — it is a $d$-dimensional vector moving through the full embedding space. The user's taste is not a single threshold to cross but a point in a high-dimensional landscape, and its trajectory traces the evolution of their aesthetic interests.
...
The neuroscience bridge is multiple behavioral readouts of the same underlying decision variable. In motor decision-making, the same evidence accumulation process drives both saccadic eye movements and manual reaching responses, but the readout thresholds and motor pathways differ. An eye movement is fast and cheap (analogous to a click — low commitment, noisy signal); a reaching movement is slower and more deliberate (analogous to a save or collection-add — higher commitment, cleaner signal). The decision variable is the same internal state, but different motor systems read it out at different thresholds and with different noise characteristics. The user tower's multi-head architecture is the same idea: one internal taste state, multiple readout heads that tap into it at different levels of commitment.
