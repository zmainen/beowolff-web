...
**The neuroscience bridge is reward prediction error.** Dopaminergic neurons in the VTA learn value representations by encoding the difference between expected reward and received reward: the temporal-difference error $\delta = r + \gamma V(s') - V(s)$. When the reward exceeds expectation, $\delta > 0$ and the value estimate increases; when it falls short, $\delta < 0$ and the estimate decreases. Contrastive learning does something structurally similar: it computes the difference between the model's current similarity estimate (how close two embeddings are) and the training signal's ground truth (whether the pair is positive or negative), and updates the model to close the gap. The model learns a "value function" over pairs — how valuable it is for two items to be near each other — and updates it based on the discrepancy between prediction and ground truth.
