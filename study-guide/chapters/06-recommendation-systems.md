...
The neuroscience bridge is direct and worth naming upfront. Collaborative filtering is, at its core, learning a value function over items from observed choices. An agent interacts with a world (the art corpus), receives signals about which interactions were rewarding (saves, purchases), and the system learns to predict which future interactions will be rewarding. This is the same problem RL solves: estimate a value function $V(s)$ over states from observed trajectories and rewards. The difference is that in recommendation the "agent" is the user, the "states" are items, the "actions" are interactions, and the "rewards" are implicit signals like dwell time and purchase. The mathematical machinery — matrix factorization, embedding learning, temporal credit assignment — overlaps significantly.
