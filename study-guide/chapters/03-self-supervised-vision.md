...
The core insight is that useful representations emerge as a byproduct of prediction. The model generates a prediction — of a masked patch, of another augmented view, of the next token in a sequence — and updates its parameters to minimize the prediction error. It is never told "this is a face" or "this is Baroque." Instead, the structure of the world is extracted from the statistical regularities that make prediction possible. Representations that capture genuine structure produce better predictions; representations that capture noise do not. The training objective acts as a filter.
...
The program's strategy follows from this layer-by-layer analysis: **keep the low layers frozen or lightly fine-tuned, and concentrate training budget on mid and high layers where art-specific signal lives.** This is the standard transfer-learning playbook, and it works because low-level visual statistics (edges, textures, spatial frequencies) are shared across domains. Edge detectors are edge detectors whether the input is a photograph or a fresco. The domain-specific knowledge — what makes a Mannerist portrait different from a Neoclassical one — lives in the higher layers, which is exactly where fine-tuning concentrates its updates.
