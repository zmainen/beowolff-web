...
The power of the shared space is that the dimensions encode features meaningful to both images and text simultaneously. A direction in the space might correspond to "outdoor scene with warm light" — and both an image of a sunset landscape and the text "golden hour over hills" project strongly along that direction. The space is not a visual space with text bolted on, or a linguistic space with images bolted on. It is a jointly learned space where both modalities contribute to and benefit from the same geometric structure.
