...
Everything in the Rasa program rests on a single architectural bet: that it is possible to represent artworks, artists, collectors, and market segments as points in a shared continuous space, such that geometric relationships between points — distances, angles, clusters — encode meaningful semantic relationships. The entire recommendation system, the cold-start mechanism, the attribution pipeline, the visualiser — all of them are downstream of this one commitment. If the embedding space is well-constructed, every downstream task becomes a geometric query. If it is poorly constructed, no amount of downstream engineering will recover what was lost.
...
The key point is that the representation is **distributed** — each axis contributes to the encoding of every object, and each object is encoded across all axes simultaneously. No single coordinate carries the meaning; the pattern does. And the representation is **learned**, not assigned. Embedding vectors do not arrive with meanings; they learn to tile the manifold of artworks through training. What the representation encodes depends entirely on the learning signal. Embeddings trained with different loss functions develop different geometries — the same set of artworks can be arranged very differently in the space depending on what the training objective considers important.
...
The answer is: nothing individually. This is the key insight about distributed representations. A single embedding dimension does not correspond to a human-interpretable feature. The semantic content lives in the geometry — in the relative positions of points — not in the individual coordinates. Just as a single pixel in an image does not "mean" anything about the scene (it is the spatial pattern of pixels that carries the visual information), a single coordinate in a 768-dimensional vector carries no isolable meaning. The meaning is in the pattern.
