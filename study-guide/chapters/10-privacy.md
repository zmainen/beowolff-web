...
**The neuroscience bridge.** If you are comfortable with signal detection theory, $\varepsilon$ controls something directly analogous to $d'$ in psychophysics. The adversary is running a detection task: "was this person's data present or absent?" The two distributions — model output with the person's data, model output without — play the role of the signal and noise distributions. $\varepsilon$ controls how far apart those distributions are. At $\varepsilon = 0$, the distributions are identical and detection is at chance — perfect privacy. As $\varepsilon$ increases, the distributions separate and the adversary's discriminability improves. The parameter $\delta$ is the probability of a catastrophic failure — an event where the guarantee simply does not hold, analogous to a lapse in a psychophysics experiment.
